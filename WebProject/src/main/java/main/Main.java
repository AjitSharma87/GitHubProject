    ...
    public static void openExcel(){
        try {
            ...
            for (int i = 0; i < workbook.getNumberOfSheets(); i++) {
                ...
                ArrayList<ReadTestData.TestDataDefined> TestData = getTestData(sheet);
                functions.addScenario(sheet.getSheetName());

                // Iterate through all steps
                for (ReadTestData.TestDataDefined t :TestData) {
                    // Perform step
                    t.Status = doStep(t);
...
//                    System.out.println(t.toString());
    ...
    private static String doStep(ReadTestData.TestDataDefined t) {
...
//                            e.printStackTrace();
    ...
    public static ArrayList<ReadTestData.TestDataDefined> getTestData(Sheet sheet){
        ArrayList<ReadTestData.TestDataDefined> mappedTestData = new ArrayList<>();
        ...
        if (!sheet.getSheetName().equals("TestDataDefined_DO_NOT_CHANGE") && !sheet.getSheetName().equals("ALM_DETAILS")){
            ...
            for (Row r: sheet){
                ReadTestData.TestDataDefined newData = new ReadTestData.TestDataDefined();
                if (r.getRowNum() != 0){
